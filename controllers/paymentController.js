...
export const buySubscription = catchAsyncError(async(req, res, next) => {
    ...
    const plan_id = process.env.PLAN_ID || "plan_MtiePKquhBOtfF"
...
export const cancelSubscription = catchAsyncError(async(req, res, next) => {
    ...
    const subscribtionId = user.subscription.id

    let refund = false

    await instance.subscriptions.cancel(subscribtionId)

    const payment = await Payment.findOne({
        razorpay_payment_id: subscribtionId
    })
